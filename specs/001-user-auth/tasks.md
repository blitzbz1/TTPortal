...
- [x] T028 [US5] Implement forgot password screen in `src/app/forgot-password.tsx` — upgrade existing `ForgotPasswordScreen` scaffold. Dark green background per spec (Screen 09 design). Lock icon, email input with mail icon, "Trimite link de resetare" button. Call `resetPassword(email)` from `useSession`. On response (success OR error), show identical success message (FR-007 enumeration protection). Add "Înapoi la conectare" back link. All strings via `useI18n`. **Test**: T027 tests pass (green)
...
- [x] T035 [US6] Implement `src/components/AuthGate.tsx` — wrapper component that checks `useSession().session`. If null, navigate to `/sign-in` with `returnTo` param set to the intended route. Use on add-venue, write-review, and edit-venue trigger points. **Test**: T032 tests pass (green)
- [x] T036 [US6] Integrate HeaderProfileIcon into app header — add to the header-right area in `src/app/(tabs)/_layout.tsx` (publicly accessible tab layout). Replace or augment existing header-right content (language toggle + add button). **Test**: write `src/app/(tabs)/__tests__/header-integration.test.tsx` — verify HeaderProfileIcon renders in the header; verify both anonymous and authenticated states display correctly
