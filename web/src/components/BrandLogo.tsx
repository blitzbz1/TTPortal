...
function BrandPinSvg({
  ...
  const style = { "--pin-color": color } as CSSProperties;
  ...
  return (
    <svg
      ...
      className={className}
      width={size}
      height={Math.round(size * 1.397)}
      viewBox="0 0 348 486"
      fill="none"
      style={style}
    >
      <defs>
        <mask id="ttp-web-pin-mask">
      ...
      <rect width="100%" height="100%" fill="var(--pin-color)" mask="url(#ttp-web-pin-mask)" />
    </svg>
  );
}
...
export default function BrandLogo({
  ...
  return (
    <span className={`inline-flex min-w-0 items-center gap-3 ${className}`} aria-label="TTPortal">
      <BrandPinSvg color={markColor} size={markSize} className={markClassName} />
      {showWordmark ? (
        <span
          className={`font-heading text-[22px] font-extrabold leading-none tracking-tight ${wordmarkClassName ?? ""}`}
