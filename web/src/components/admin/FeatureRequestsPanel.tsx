...
const categories = ["General", "Venues", "Events", "Social", "Leaderboards", "UX", "Equipment"];

interface FeatureRequestsPanelProps {
  onCountChange?: (n: number) => void;
}

export function FeatureRequestsPanel({ onCountChange }: FeatureRequestsPanelProps) {
  ...
  const fetchFeatures = useCallback(async () => {
    ...
    const list = (data ?? []) as AdminFeature[];
    setFeatures(list);
    setLoading(false);
  ...
  function startEdit(f: AdminFeature) {
    setEditingId(f.id);
    setEditForm({ title: f.title, description: f.description, category: f.category });
  }
  ...
  async function saveEdit(id: string) {
    await supabase
      ...
      .eq("id", id);
    setFeatures((prev) => prev.map((f) => (f.id === id ? { ...f, ...editForm } : f)));
    setEditingId(null);
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex w-full max-w-[360px] items-center gap-2 rounded-[10px] border border-ink-200 bg-paper px-3 py-2.5">
          ...
          <table className="w-full text-[13.5px]">
            <thead>
              <tr className="border-b border-ink-100 bg-moss-50/50">
                <th className="kicker px-4 py-3 text-left text-ink-600">{t("colTitle")}</th>
                ...
                >
                  ...
                  <td className="px-4 py-3 align-top">
                    <select
                      value={f.status}
                      onChange={(e) =>
                        handleStatusChange(f.id, e.target.value as FeatureStatus)
                      }
