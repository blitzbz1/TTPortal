-- Migration: 037_recreate_feature_requests
-- Migration 027 is recorded as applied in supabase_migrations but its DDL
-- never reached the database (the table is missing in prod). This migration
-- re-runs the original schema. It is fully idempotent (IF NOT EXISTS,
-- OR REPLACE, DROP POLICY IF EXISTS) so it is safe whether or not 027 ran.
