...
BEGIN;
INSERT INTO countries (code,name,active) VALUES ('DZ','Algeria',true) ON CONFLICT (code) DO NOTHING;
INSERT INTO cities (name,country_code,country_name,admin_area,county,lat,lng,zoom,active,expansion_status) VALUES
  ('Hadjout','DZ','Algeria','Tipaza','Hadjout',36.51257,2.41382,12,false,'community_review')
...
FROM (VALUES
  ('Salle de Tennis de Table','parc_exterior','Hadjout','Hadjout',NULL,'Hadjout',36.51588,2.41561,1,true,false,NULL,NULL,ARRAY['exterior'],true,false)
...
JOIN cities c ON c.country_code='DZ' AND c.name=v.city
